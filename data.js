...
function getStudyStats() {
    try {
        const stats = JSON.parse(localStorage.getItem(STORAGE_KEYS.STUDY_STATS));
        if (!stats) return { totalStudyDays: 0, studyDates: [] };
        return stats;
    } catch (e) {
        return { totalStudyDays: 0, studyDates: [] };
    }
}
...
function calculateOverallStats() {
    try {
        ...
        const correctQuestions = Object.values(userAnswers).filter(a => a && a.isCorrect).length;
        ...
        return { totalQuestions: 0, answeredQuestions: 0, correctQuestions: 0, accuracyRate: 0 };
    }
}
